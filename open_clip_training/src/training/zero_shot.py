...
def run(model, classifier, dataloader, args):
    ...
    with torch.no_grad():
        ...
        for images, target, masks in tqdm(dataloader, unit_scale=args.batch_size):
            ...
            with autocast():
                ...
                if args.distributed and not args.horovod:
                    ...
                    for image, mask in zip(images, masks):
                        image_feature = model.encode_image(image.unsqueeze(0), mask.unsqueeze(0))
                        image_features.append(image_feature)
                    image_features = torch.cat(tuple(image_features), axis=0).to(args.device)
