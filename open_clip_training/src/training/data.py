...
from .imagenet_zeroshot_data import openai_imagenet_template as templates
...
class ADEDataset(Dataset):
    def __init__(self, val_folder, transforms):
        logging.debug(f'Loading ade data.')

        self.masks = []
        for root, dirs, _ in os.walk(val_folder):
            for dir in dirs:
                for d, _, files in os.walk(os.path.join(root,dir)):
                    for file in files:
                        if file.lower().endswith(tuple(['.jpg'])):
                            self.masks.append(os.path.join(root, d, file))
        self.images = ['ADEChallengeData2016/images/validation/'+m[-20:] for m in self.masks]
        self.classes = [m[-24:-21] for m in self.masks]
        self.transforms = transforms
        logging.debug('Done loading data.')
    ...
    def __getitem__(self, idx):
        mask_filepath = self.masks[idx]
        image_filepath = os.path.join(os.getenv('SLURM_TMPDIR'),'openclip_data',str(self.images[idx]))
        masks = self.transforms(Image.open(str(mask_filepath)))
        images = self.transforms(Image.open(str(image_filepath)))
        classes = torch.tensor(int(self.classes[idx]))
        return masks, classes, images

def get_ade_val(args, preprocess_fns):
    _, preprocess_val = preprocess_fns

    data_path = args.ade_val
    dataset = ADEDataset(data_path,transforms=preprocess_val) #datasets.ImageFolder(data_path, transform=preprocess_val)
...
def get_data(args, preprocess_fns, epoch=0):
    ...
    if args.ade_val is not None:
        data["ade-val"] = get_ade_val(args, preprocess_fns)
