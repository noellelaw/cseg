...
class VisualTransformer(nn.Module):
    ...
    def get_vit_embedding(self, x: torch.Tensor, m: torch.Tensor = None):
        ...
        if not self.mask_emb_depth: 
          ...
          x = self.transformer(x)
          x = x.permute(1, 0, 2)  # LND -> NLD
          x_b, x_c, g_squared = x.shape
          grid = int(np.sqrt(g_squared))
          vit_out = x.reshape(x_b, x_c, grid, grid)  # shape = [*, width, grid ** 2]
          pos = self.positional_embedding.reshape(-1, x_c, grid, grid)[:,1:]
          pos_out = pos.repeat(x_b, 1,1,1)
