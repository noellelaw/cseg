...
class OVSeg(MaskFormer):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            #print('SEMSEG OUTPUTS: ',mask_cls_results.shape, mask_pred_results.shape)
            # upsample masks
            ...
            pred_cls, processed_results = [], []
            idx = 0
            num_batches = len(batched_inputs)
            for i in range(num_batches):
                height = images.image_sizes[i][0]
                width = images.image_sizes[i][1]
                mask_pred_result = sem_seg_postprocess(
                    mask_pred_results[i], images.image_sizes[i], height, width
                )
                image = batched_inputs[i]["image"].to(self.device)

                r, regions, clip_logits, valid_flag = self.semantic_inference(
                    mask_cls_results[i], mask_pred_results[i], image, class_names
                )
                map_back_clip_cls = mask_cls_results[i].new_ones(mask_cls_results[i].shape)
                map_back_clip_cls[valid_flag] = clip_logits
                mask_cls_results[idx] = map_back_clip_cls

                height = batched_inputs[i].get("height", images.image_sizes[i][0])
                width = batched_inputs[i].get("width", images.image_sizes[i][1])
                r = sem_seg_postprocess(r, images.image_sizes[i], height, width)
                processed_results.append({"sem_seg": r})

                # panoptic segmentation inference
                if self.panoptic_on:
                    panoptic_r = self.panoptic_inference(
                        mask_cls_results[i], mask_pred_results[i]
                    )
            ...
            for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
                ...
                map_back_clip_cls = mask_cls_result.new_ones(mask_cls_result.shape)
                map_back_clip_cls[valid_flag] = clip_logits
                mask_cls_results[idx] = map_back_clip_cls
            ...
            clip_outputs = {"pred_masks": mask_pred_results, 
                            "pred_logits": mask_cls_results}
            losses = self.criterion(clip_outputs, targets)
    ...
    def semantic_inference(self, mask_cls, mask_pred, image, class_names):
        ...
        if self.clip_ensemble:
            clip_logits, regions, valid_flag = self.clip_adapter(
                image, class_names, mask_pred, normalize=True
            )
            clip_cls = clip_logits.clone()
            if clip_cls is None:
        ...
        return semseg, regions, clip_logits, valid_flag
