...
class GeneralizedSemSegEvaluator(SemSegEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            pred = np.array(output, dtype=np.intc)
            with PathManager.open(
                self.input_file_to_gt_file[input["file_name"]], "rb"
            ) as f:
                gt = np.array(Image.open(f), dtype=np.intc)
    ...
    def evaluate(self):
        ...
        * Pixel Accuracy (pACC)
        ...
        acc = np.full(self._num_classes, np.nan, dtype=np.single)
        iou = np.full(self._num_classes, np.nan, dtype=np.single)
        tp = self._conf_matrix.diagonal()[:-1].astype(np.single)
        pos_gt = np.sum(self._conf_matrix[:-1, :-1], axis=0).astype(np.single)
        class_weights = pos_gt / np.sum(pos_gt)
        pos_pred = np.sum(self._conf_matrix[:-1, :-1], axis=1).astype(np.single)
        acc_valid = pos_gt > 0
        ...
        if self._evaluation_set is not None:
            for set_name, set_inds in self._evaluation_set.items():
                iou_list = []
                set_inds = np.array(set_inds, np.intc)
