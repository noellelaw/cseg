...
def build_model(state_dict: dict, mask_prompt_depth: int = 0):
    ...
    if vit:
        ...
        vision_layers = len(
            [
                k
                for k in state_dict.keys()
                if k.startswith("clip_adapter.clip_model.visual.") and k.endswith(".attn.in_proj_weight")
            ]
        )
        vision_patch_size = state_dict["clip_adapter.clip_model.visual.conv1.weight"].shape[-1]
        grid_size = round(
            (state_dict["clip_adapter.clip_model.visual.positional_embedding"].shape[0] - 1) ** 0.5
        )
        image_resolution = vision_patch_size * grid_size
    ...
    if mpt_vit:
      ...
      embed_dim = state_dict["clip_adapter.clip_model.text_projection"].shape[1]
      context_length = state_dict["clip_adapter.clip_model.positional_embedding"].shape[0]
      vocab_size = state_dict["clip_adapter.clip_model.token_embedding.weight"].shape[0]
      transformer_width = state_dict["clip_adapter.clip_model.ln_final.weight"].shape[0]
      transformer_heads = transformer_width // 64
      transformer_layers = len(
          set(
              k.split(".")[2]
              for k in state_dict
              if k.startswith(f"clip_adapter.clip_model.transformer.resblocks")
          )
      )
      ...
      for key in ["clip_adapter.clip_model.input_resolution", "clip_adapter.clip_model.context_length", "clip_adapter.clip_model.vocab_size"]:
